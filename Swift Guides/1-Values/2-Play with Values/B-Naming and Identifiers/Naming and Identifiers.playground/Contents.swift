...
//:
//: For example, if another person is bringing a rabbit, keep track of the change by updating the `Number Of Rabbits` from `5` to `6` and change the `5` to a `6` everywhere else it appears (`Total Number Of Animals` and `Total Number Of Mammals`).
//:
