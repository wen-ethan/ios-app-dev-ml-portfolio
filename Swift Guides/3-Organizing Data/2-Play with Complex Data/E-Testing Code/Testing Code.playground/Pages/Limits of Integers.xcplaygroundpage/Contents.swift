/*:
 ...
 Below is the code for `PennyBank`.
 */
...
/*:
 ...
 The "U" in `UInt8` stands for "unsigned." An unsigned integer is always positive. Penny banks hold real coins, so there's no need to allow for a negative balance.
 
 Since this bank counts its pennies with `UInt8`, the number of pennies it can contain is limited by the capacity of an 8-bit number. Values of `UInt8` range from 0 to 255.
